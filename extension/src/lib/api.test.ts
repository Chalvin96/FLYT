...
describe("resolveWord", () => {
  ...
  it("test_resolve_word_given_expression_suggestion_expect_candidate_returned", async () => {
    const expression = "få [stelle|lage] i stand";
    const candidate = {
    ...
    fetch.mockResolvedValue(
      jsonResponse({ query: expression, candidates: [candidate] }),
    );

    const result = await resolveWord(expression);

    expect(fetch).toHaveBeenCalledWith(
      `${ORIGIN}/lexicons/resolve?word=f%C3%A5%20%5Bstelle%7Clage%5D%20i%20stand`,
    );
